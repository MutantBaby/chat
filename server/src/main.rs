...
fn main() {
    let server: TcpListener = TcpListener::bind(LOCAL).expect("Listener failed to bind");
    server
            ...
            thread::spawn(move || loop {
                ...
                match socket.read_exact(&mut buff) {
                    Ok(_) => {
                        ...
                        let msg: String = String::from_utf8(msg).expect("Invalid utf8 message");
        ...
        if let Ok(msg) = rx.try_recv() {
            clients = clients
                ...
                .filter_map(|mut client| {
                    ...
                    client.write_all(buff.as_ref()).map(|_| client).ok()
                })
                .collect::<Vec<_>>();
        }
